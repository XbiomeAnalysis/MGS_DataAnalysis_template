...
# initiate graph
graph [layout = dot, rankdir = TB, label = '[metaphlan|metadata|tree]\n\n', labelloc = t]
...
[1]: paste0('Importing data: import_metaphlan_taxa()/import_metaphlan2()/summarize_phyloseq()')
[2]: paste0('Quality Control(RefCheck/Remove): run_RefCheck()/get_GroupPhyloseq()')
[3]: paste0('Data processing(Extract specific taxonomic level): summarize_taxa()') 
[4]: paste0('Data processing(Filter&trim): run_filter()/run_trim()') 
[5]: paste0('GlobalView(Alpha diveristy): run_alpha_diversity()/plot_boxplot()') 
[6]: paste0('GlobalView(Beta diveristy): run_beta_diversity()/run_permanova()/run_ordination()/plot_Ordination()') 
[7]: paste0('GlobalView(microbial composition): plot_StackBarPlot()/plot_core_taxa()/core_members()')
[8]: paste0('DifferentialAnalysis: run_lefse()/plot_lefse()/run_wilcox()/plot_volcano()')
")
```
